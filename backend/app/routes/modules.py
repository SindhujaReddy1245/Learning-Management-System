...
from app.repositories.module_pdfs import (
...
async def post_module_quiz_attempt(module_id: str, attempt: QuizAttemptCreate) -> QuizAttemptOut:
    # Enforce max 5 attempts per student per module
    attempts = await get_attempts_for_student(module_id, attempt.studentId)
    if len(attempts) >= 5:
        raise HTTPException(status_code=429, detail="Maximum number of attempts reached for this quiz")
    quiz = await repo_get_module_quiz(module_id)
    if quiz is None:
        raise HTTPException(status_code=404, detail="Quiz not found for this module")
